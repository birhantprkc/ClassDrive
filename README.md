...
- 文件预览：支持图片、PDF、音视频、文本等常见格式；HTML 网页在沙箱环境中在线预览效果，Markdown 在线渲染预览；文本文件可在线编辑。
...
- 班级管理：创建、编辑、删除班级，开启或关闭学生注册码，按班级隔离资料、学生、作业和提交；支持开启"考试锁定"，锁定期间禁止本班学生下载和预览历史提交，防止考试时复用旧作业。
...
- 作业提交：按教师要求在弹窗内选择文件或文件夹，确认后才会提交；截止前可增量添加到当前提交，同路径或同名文件会替换，已有文件会保留。常用格式覆盖 Office/PDF/HTML/Markdown/CSV/JSON/图片/音视频/压缩包等，AI 生成的网页与 Markdown 报告可直接提交并在线上预览效果。
